...
def plot_genotype_reconstruction_diagnostics(
    x_true,
    x_hat,
    obs_mask=None,
    genotype_likelihood: str = "mse",
    out_dir: str = ".",
    filename: str = "Genotype_Reconstruction_Diagnostics.pdf",
    max_points: int = 100000,
    random_state: int = 21,
) -> None:
    """
    Compact reconstruction diagnostic that works across genotype likelihoods.

    x_hat is expected to be on the common genotype scale:
      MSE         -> raw reconstruction
      Bernoulli   -> P(X=1|z)
      Categorical -> E[X|z]

    To avoid plotting tens of millions of SNP/sample cells, observed entries
    are randomly subsampled for visualization only.
    """
    _ensure_dir(out_dir)

    x_true = _to_numpy(x_true).astype(float)
    x_hat = _to_numpy(x_hat).astype(float)

    if x_true.shape != x_hat.shape:
        raise ValueError(
            f"x_true shape {x_true.shape} != x_hat shape {x_hat.shape}"
        )
    ...
    if obs_mask is None:
        ...
        mask = (
            _to_numpy(obs_mask).astype(bool)
            & np.isfinite(x_true)
            & np.isfinite(x_hat)
        )

    true_flat = x_true[mask]
    pred_flat = x_hat[mask]
    ...
    classes = np.unique(true_flat)
    classes = classes[np.isfinite(classes)]

    fig, ax = plt.subplots(figsize=(6.5, 4.5))

    if len(classes) <= 10 and np.allclose(classes, np.round(classes)):
        grouped = [
            pred_flat[np.isclose(true_flat, c)]
            for c in classes
        ]
        ax.boxplot(
            grouped,
            tick_labels=[str(int(round(c))) for c in classes],
            showfliers=False,
        )
        ax.set_xlabel("Observed genotype class")
        ax.set_ylabel(
            "Predicted genotype probability / expected dosage"
            if genotype_likelihood != "mse"
            else "Predicted genotype value"
        )
    else:
        ax.scatter(
            true_flat,
        ...
        lo = min(np.min(true_flat), np.min(pred_flat))
        hi = max(np.max(true_flat), np.max(pred_flat))
        ax.plot(
            [lo, hi],
            [lo, hi],
            linestyle="--",
        ...
        ax.set_xlabel("Observed genotype")
        ax.set_ylabel("Reconstructed genotype")

    ax.set_title(
        f"Genotype reconstruction ({genotype_likelihood})"
    )
    fig.tight_layout()
    fig.savefig(
        os.path.join(out_dir, filename),
...
def plot_bernoulli_genotype_calibration(
    x_true,
    x_prob,
    obs_mask=None,
    out_dir: str = ".",
    filename: str = "Bernoulli_Genotype_Calibration.pdf",
    n_bins: int = 10,
) -> None:
    """
    Calibration of P(X=1|z) for binary SNP reconstruction.

    Within bins of predicted ALT probability, plot the observed fraction of
    genotype value 1. This is a decoder-reconstruction diagnostic, not a
    phenotype-calibration plot.
    """
    _ensure_dir(out_dir)

    x_true = _to_numpy(x_true).astype(float)
    x_prob = _to_numpy(x_prob).astype(float)

    if x_true.shape != x_prob.shape:
        raise ValueError(
            f"x_true shape {x_true.shape} != x_prob shape {x_prob.shape}"
        )

    if obs_mask is None:
        mask = np.isfinite(x_true) & np.isfinite(x_prob)
    else:
        mask = (
            _to_numpy(obs_mask).astype(bool)
            & np.isfinite(x_true)
    ...
    if not np.all(np.isin(np.unique(y), [0.0, 1.0])):
        raise ValueError(
            "Bernoulli calibration requires observed SNP values in {0,1}."
        )
    ...
    for lo, hi in zip(edges[:-1], edges[1:]):
        ...
        if not np.any(idx):
            continue
        centers.append(float(np.mean(p[idx])))
        observed.append(float(np.mean(y[idx])))
    ...
    ax.set_xlim(0, 1)
    ax.set_ylim(0, 1)
    ax.set_xlabel("Predicted P(genotype = 1)")
    ax.set_ylabel("Observed fraction genotype = 1")
    ax.set_title("Bernoulli SNP reconstruction calibration")
    ax.legend(frameon=False)
    fig.tight_layout()
    fig.savefig(
        os.path.join(out_dir, filename),
    ...
    pd.DataFrame(
        {
            "mean_predicted_probability": centers,

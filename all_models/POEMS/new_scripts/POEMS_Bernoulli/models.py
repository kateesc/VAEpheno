...
class SNPPOEMSVAE(nn.Module):
    ...
    decoder_type="poems"
        ...
        Supported genotype_likelihood: "mse" or "bernoulli".
    ...
    * For categorical reconstruction, decode(z) returns E[X|z] for convenient
    ...
    def __init__(
        ...
        if decoder_type == "poems" and genotype_likelihood == "categorical":
            raise ValueError(
                "POEMS categorical reconstruction is not implemented in this "
                "controlled experiment. Use POEMS with MSE/Bernoulli, or "
                "dense_mlp for categorical reconstruction."
            )
    ...
    def _decode_poems_raw(self, z):
        """
        Raw scalar POEMS decoder output per SNP.

        Interpretation depends on genotype_likelihood:
          * MSE       -> real-valued reconstruction (clamped later exactly as
                         in the original C4 implementation).
          * Bernoulli -> Bernoulli logit eta_j(z), left unclamped because
    ...
    def decode_logits(self, z):
        ...
        """
        if self.decoder_type == "poems":
            raw = self._decode_poems_raw(z)
            if self.genotype_likelihood == "mse":
                # Preserve the original C4 POEMS+MSE behavior exactly.
                return torch.clamp(raw, min=-10.0, max=10.0)
            # For Bernoulli, this is eta_j(z), the raw logit.
            return raw

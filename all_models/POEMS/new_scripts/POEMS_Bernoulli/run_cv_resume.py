...
def print_resume_status(args, completed):
    ...
    if args.cv_strategy in ["random", "label_stratified"]:
        ...
        # pca_blocked uses GroupKFold without repeats. The actual number of
        # folds cannot exceed the configured number of structure clusters.
        expected_n_folds = min(args.n_splits, args.n_structure_clusters)
...
def _resolved_cv_config(args, defaults):
    """
    Configuration fingerprint for safe resume.

    A fold number is only reusable when the split-generating and model-defining
    settings are identical. This file prevents accidental mixing of runs.
    """
    return {
        ...
        "enc_hidden_dim": int(args.enc_hidden_dim),
        "dec_hidden_dim": int(args.dec_hidden_dim),
        "dec_hidden_dim2": int(args.dec_hidden_dim2),
        "decoder_type": args.decoder_type,
        "genotype_likelihood": args.genotype_likelihood,
        "n_genotype_classes": int(args.n_genotype_classes),
        "regressor_type": args.regressor_type,
        "trait_likelihood": args.trait_likelihood,
        "n_structure_pcs": int(args.n_structure_pcs),
        "n_structure_clusters": int(args.n_structure_clusters),
        "force_trait_outliers_train": bool(
            not args.no_force_trait_outliers_train
        ),
...
def _prepare_cv_config(args, defaults):
    path = Path(args.out_dir) / "cv_run_config.json"
    current = _resolved_cv_config(args, defaults)

    if args.resume:
        if not path.exists():
            raise RuntimeError(
                f"--resume requested but {path} is missing. "
                "Refusing to resume because the original run configuration "
                "cannot be verified."
            )
        old = json.loads(path.read_text())
        if old != current:
            old_s = json.dumps(old, indent=2, sort_keys=True)
            cur_s = json.dumps(current, indent=2, sort_keys=True)
            raise RuntimeError(
                "Resume configuration mismatch. Refusing to mix incompatible "
...
def main():
    ...
    if (args.decoder_type, args.genotype_likelihood) not in allowed:
        raise ValueError(
            f"Unsupported decoder/likelihood combination: "
            f"{args.decoder_type}+{args.genotype_likelihood}"
        )
    ...
    # The old POEMS L1 parameter acts specifically on the SNP-factor gate W.
    # It has no equivalent in the dense decoder ablation.
    if args.decoder_type == "dense_mlp":
        if float(defaults["decoder_l1_lambda"]) != 0.0:
            print(
                "Dense decoder selected: overriding decoder_l1_lambda to 0. "
                "The POEMS gate-W penalty is not transferable to dense_mlp."
            )
        defaults["decoder_l1_lambda"] = 0.0

    os.makedirs(args.out_dir, exist_ok=True)
    config_path = _prepare_cv_config(args, defaults)
    print(f"CV configuration: {config_path}")
    ...
    for trainval_idx, test_idx, groups in make_cv_splits(args, data):
        ...
        for ens in range(args.ensemble_seeds):
            ...
            result["decoder_type"] = args.decoder_type
            result["genotype_likelihood"] = args.genotype_likelihood
            result["dec_hidden_dim"] = args.dec_hidden_dim
            result["dec_hidden_dim2"] = args.dec_hidden_dim2
            result["n_genotype_classes"] = args.n_genotype_classes

...
def infer_snp_structure(x_raw, train_idx=None, n_pcs=10, n_clusters=3, random_state=21):
    ...
    Uses train-only SNP means if train_idx is provided; otherwise uses all samples.
    """
    if train_idx is None:
        fit_x = x_raw
    else:
        fit_x = x_raw[train_idx]
    means = np.nanmean(fit_x, axis=0)
    means = np.where(np.isnan(means), 0.0, means)
    x_filled = np.where(np.isnan(x_raw), means[None, :], x_raw)

    scaler = StandardScaler(with_mean=True, with_std=True)
    x_scaled = scaler.fit_transform(x_filled)
    n_pcs_eff = min(n_pcs, x_scaled.shape[0] - 1, x_scaled.shape[1])
    pca = PCA(n_components=n_pcs_eff, random_state=random_state)
    pcs = pca.fit_transform(x_scaled)

    n_clusters_eff = min(n_clusters, len(x_scaled))
    if n_clusters_eff >= 2:
        clusters = KMeans(n_clusters=n_clusters_eff, n_init=50, random_state=random_state).fit_predict(pcs)
    else:
...
def run_epoch(model, loader, optimizer, beta_kl, alpha_trait, decoder_l1_lambda, train=True):
    model.train() if train else model.eval()
    deterministic = not train

    totals = {"loss": 0.0, "recon": 0.0, "kl": 0.0, "trait": 0.0, "w_l1": 0.0}
    n_batches = 0
    store = {k: [] for k in ["mu", "z", "y", "y_raw", "y_mu", "y_logvar", "y_std", "labels", "x_raw", "x_in", "x_hat", "obs_mask", "trait_mask"]}
    sample_names_all = []
    ...
    for batch in loader:
        ...
        with torch.set_grad_enabled(train):
            out = model(x_in, deterministic=deterministic)
            for key in ["mu", "logvar", "z", "x_hat", "y_mu"]:
                if torch.isnan(out[key]).any():
                    raise ValueError(f"NaN detected in model output: {key}")

            recon = masked_mse_loss(out["x_hat"], x_raw, obs_mask)
            kl = gaussian_kl(out["mu"], out["logvar"]).mean()
            if out["y_logvar"] is None:
                trait_loss = masked_trait_mse_loss(out["y_mu"], y, trait_mask)
            else:
                trait_loss = masked_trait_gaussian_nll(out["y_mu"], out["y_logvar"], y, trait_mask)
            w_l1 = decoder_l1_lambda * torch.abs(model.W).mean()
            loss = recon + beta_kl * kl + alpha_trait * trait_loss + w_l1
        ...
        if out["y_logvar"] is None:
            store["y_logvar"].append(torch.full_like(out["y_mu"], float("nan")).detach().cpu())
            store["y_std"].append(torch.full_like(out["y_mu"], float("nan")).detach().cpu())
        else:
            store["y_logvar"].append(out["y_logvar"].detach().cpu())
            store["y_std"].append(out["y_std"].detach().cpu())
    ...
    out["sample_names"] = sample_names_all
    out["y_hat"] = out["y_mu"]
    return out

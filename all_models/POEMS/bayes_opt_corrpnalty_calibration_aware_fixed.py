#!/usr/bin/env python3
"""
Calibration-aware Bayesian optimization for the supervised POEMS SNP-VAE.
...
def _trial_params(trial: optuna.Trial, args: argparse.Namespace) -> Dict[str, Any]:
    return {
        "lr": trial.suggest_float("lr", args.lr_min, args.lr_max, log=True),
        "wd": trial.suggest_float("wd", args.wd_min, args.wd_max, log=True),
        "latent_dim": trial.suggest_categorical("latent_dim", args.latent_dims),
        "alpha_trait": trial.suggest_float("alpha_trait", args.alpha_min, args.alpha_max, log=True),
        "beta_kl": trial.suggest_float("beta_kl", args.beta_min, args.beta_max, log=True),
        "decoder_l1_lambda": trial.suggest_float(
            "decoder_l1_lambda", args.l1_min, args.l1_max, log=True
        ),
        "dropout": trial.suggest_float("dropout", args.dropout_min, args.dropout_max),
        "early_stop_metric": trial.suggest_categorical(
            "early_stop_metric", args.early_stop_metrics
        ),
    }
...
def validate_warm_start_params(
    ...
    try:
        checks = [
            ...
            _within(float(params["alpha_trait"]), args.alpha_min, args.alpha_max),
            _within(float(params["beta_kl"]), args.beta_min, args.beta_max),
            _within(float(params["decoder_l1_lambda"]), args.l1_min, args.l1_max),
            _within(float(params["dropout"]), args.dropout_min, args.dropout_max),
            str(params["early_stop_metric"]) in set(args.early_stop_metrics),
        ]
...
def enqueue_warm_start_trials(
    ...
    for _, row in df.iterrows():
        params = {
            ...
            "decoder_l1_lambda": float(row["decoder_l1_lambda"]),
...
def build_study_config(
    ...
    return {
        ...
        "search_space": {
            ...
            "decoder_l1_lambda": [args.l1_min, args.l1_max],
...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(
        description=(
            ...
            "for the supervised POEMS SNP-VAE."
        )
    )
